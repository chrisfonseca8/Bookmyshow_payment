      ...
      <div className="fixed top-0 left-0 w-full z-30">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[12px] text-gray-500 mt-0.5">
                PVR: Nucleus Mall, Ranchi &nbsp;|&nbsp; {time ? (new Date(time).toLocaleDateString('en-US', { weekday: 'short', day: 'numeric', month: 'long', year: 'numeric' }) + ' | ' + new Date(time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })) : 'Loading Time...'}
              </p>
            </div>
          </div>
        ...
        <div className="bg-white border-b border-gray-200 py-3 overflow-x-auto scb bg-opacity-95 backdrop-blur-sm">
          <div className="flex items-center gap-3 px-4 md:px-6 w-max">
            {time ? (
              <button className="min-w-[85px] px-4 py-1.5 text-xs text-center rounded-[3px] border bg-[#f5b53f] border-[#f5b53f] text-white font-medium">
                {new Date(time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
              </button>
            ) : (
              <button className="min-w-[85px] px-4 py-1.5 text-xs text-center rounded-[3px] border border-gray-300 text-gray-400 font-medium">
                Loading...
              </button>
            )}
          </div>
          ...
          <div className="w-full overflow-x-auto pb-10 custom-scrollbar">
            <div className="min-w-[600px] mx-auto px-4 md:px-12 flex flex-col items-center">
              {seattypes.map((type) => {
                ...
                // Get price dynamically from the first seat in this tier (fallback to 250)
                const price = typeSeats.length > 0 ? typeSeats[0].price || 250 : 250;
